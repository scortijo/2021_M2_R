...
Nous avons maintenant une colonne `condition`, qui contient le nom des colonnes que nous avons regroupées et une colonne `expression` qui contient les valeurs des colonnes (le niveu d'expression des gènes). Ce n'est pas encore exactement ce dont nous avons besoin car la colonne `condition` contient toutes les informations de l'expérience, mais nous voulons une colonne pour chacune de ces informations: méthode de normalisation, température, durée de traitement.
